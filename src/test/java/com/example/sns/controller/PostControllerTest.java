...
public class PostControllerTest {
    ...
    @Test
    @WithAnonymousUser
    void 좋아요_클릭시_로그인하지_않은_경우() throws Exception{
        mockMvc.perform(post("/api/v1/posts/1/likes")
                        .contentType(MediaType.APPLICATION_JSON)
                ).andDo(print())
                .andExpect(status().isUnauthorized());
    ...
    void 좋아요_클릭시_게시물이_없는_경우() throws Exception{
        //mocking
        doThrow(new SnsApplicationException(ErrorCode.POST_NOT_FOUNDED)).when(postService).like(any(),any());
    ...
    void 댓글_작성시_게시물이_없는_경우() throws Exception{
        //mocking
        doThrow(new SnsApplicationException(ErrorCode.POST_NOT_FOUNDED)).when(postService).comment(any(),any(),any());
